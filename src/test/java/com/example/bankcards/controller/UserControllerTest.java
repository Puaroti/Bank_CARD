...
/**
 * Тесты пользовательских эндпоинтов {@code /api/user} для операций с картами и переводами.
 * <p>
 * Используется SpringBootTest + MockMvc с подменой JWT-фильтра (сквозной проход),
 * чтобы сосредоточиться на авторизации по ролям и корректности HTTP-ответов.
 */
@SpringBootTest
...
class UserControllerTest {
    ...
    /**
     * Настройка заглушки для JwtAuthenticationFilter: не блокирует запросы,
     * а делегирует выполнение дальше по цепочке.
     */
    @BeforeEach
    ...
    /**
     * Пользователь с ролью USER может получить собственные карты.
     ...
     * Ожидания: HTTP 200 OK и возвращение DTO карты со статусом BLOCKED.
     */
    @Test
    ...
    /**
     * Пользователь с ролью USER может запросить разблокировку своей карты.
     * Ожидания: HTTP 200 OK и возвращение DTO карты со статусом ACTIVE.
     */
    @Test
    ...
    /**
     * Бизнес-правило: нельзя разблокировать истёкшую карту.
    ...
    /**
     * Пользователь с ролью USER может получить баланс своей карты.
     * Ожидания: HTTP 200 OK.
     */
    @Test
    ...
    /**
     * Пользователь с ролью USER может выполнить перевод между своими картами.
     * Ожидания: HTTP 200 OK и отсутствие исключений при корректном запросе.
     */
    @Test
